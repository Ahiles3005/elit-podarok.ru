...
?>
<?
//TODO: Фикс костыльный. Переделать как разберемся до конца.
preg_match('/page-(\d+)/', $_SERVER["REQUEST_URI"], $output_array);
if (!empty($output_array)) {
    $page = str_replace('/'.$output_array[0].'/', '/', $_SERVER["REQUEST_URI"]);

    if(stripos($arResult['VARIABLES']['SECTION_CODE_PATH'], 'filter/') === false) {
        $sectionSTR = file_get_contents('http://'.$_SERVER['SERVER_NAME'].$page.'?CUSTOM_GET_SECTION=Y');
        $arResult['VARIABLES']['SECTION_CODE_PATH'] = $sectionSTR;
    }
    if(empty($arResult['smartFilterPath'])) {
        $filterSTR = file_get_contents('http://'.$_SERVER['SERVER_NAME'].$page.'?CUSTOM_GET_FILTER=Y');
        $arResult['smartFilterPath'] = $filterSTR;
    }
}

if ($_REQUEST['CUSTOM_GET_SECTION'] == 'Y') {
    $APPLICATION->RestartBuffer();
    echo $arResult['VARIABLES']['SECTION_CODE_PATH'];
    die();
}
if ($_REQUEST['CUSTOM_GET_FILTER'] == 'Y') {
    $APPLICATION->RestartBuffer();
    echo $arResult['smartFilterPath'];
    die();
}
?>
