...
<?if($arResult["NavPageCount"] > 1):?>
    ...
    $pos = strpos($arResult["NavQueryString"], "?");
    $strNavQueryString = ($arResult["NavQueryString"] != "" ? ( !$pos ? "?" : "" ) . $arResult["NavQueryString"] : "");
    ...
    if(!empty($_REQUEST["find_section"])) {
        $arResult["sUrlPath"] = "/catalog/".$_REQUEST["find_section"]."/";
        $arResult["sUrlPathParams"] = "/catalog/".$_REQUEST["find_section"]."/?";
    }
    ?>
        ...
        <div class="nums">
            <ul class="flex-direction-nav">
                <?if(!$bPrevDisabled):?>
                    ...
                    $url = ($page ? 'page-'.$page.'/'.$strNavQueryString : $strNavQueryStringFull);?>
                ...
                <?endif;?>
                <?if(!$bNextDisabled):?>
                    <?$APPLICATION->AddHeadString('<link rel="next" href="'.$arResult["sUrlPath"].'page-'.$strNavQueryString.($arResult["NavPageNomer"]+1).'/"  />', true);?>
                    <li class="flex-nav-next "><a href="<?=$arResult["sUrlPath"]?>page-<?=($arResult["NavPageNomer"]+1)?>/<?=$strNavQueryString?>" class="flex-next"></a></li>
                <?endif;?>
            </ul>
            <?if($arResult["nStartPage"] > 1):?>
                <a href="<?=$arResult["sUrlPath"]?>page-1/<?=$strNavQueryString?>" class="dark_link">1</a>
                <?if(($arResult["nStartPage"] - $count_item_dotted) > 1):?>
                    <span class='point_sep'></span>
                <?elseif(($firstPage = $arResult["nStartPage"]-1) > 1 && $arResult["nStartPage"] !=2):?>
                    <a href="<?=$arResult["sUrlPath"]?>page-<?=$firstPage?>/<?=$strNavQueryString?>"><?=$firstPage?></a>
                <?endif;?>
            <?endif;?>
            <?while($arResult["nStartPage"] <= $arResult["nEndPage"]):?>
                ...
                <?else:?>
                    <a href="<?=$arResult["sUrlPath"]?>page-<?=$arResult["nStartPage"]?>/<?=$strNavQueryString?>" class="dark_link"><?=$arResult["nStartPage"]?></a>
                <?endif;?>
                <?$arResult["nStartPage"]++;?>
            <?endwhile;?>
            <?if($arResult["nEndPage"] < $arResult["NavPageCount"]):?>
                ...
                <?elseif(($lastPage = $arResult["nEndPage"]+1) < $arResult["NavPageCount"]):?>
                    <?=$arResult["sUrlPath"]."w"?>
                    <a href="<?=$arResult["sUrlPath"]?>page-<?=$lastPage?>/<?=$strNavQueryString?>"><?=$lastPage?></a>
                <?endif;?>
                <?=$strNavQueryString . "q"?>
                <a href="<?=$arResult["sUrlPath"]?>page-<?=$arResult["NavPageCount"]?>/<?=$strNavQueryString?>" class="dark_link"><?=$arResult["NavPageCount"]?></a>
            <?endif;?>
