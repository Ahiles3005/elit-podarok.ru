...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<?}?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){
            //$arItem["DETAIL_PAGE_URL"] = str_replace("/catalog", "", $arItem["DETAIL_PAGE_URL"]);
		    ?>
							...
							<div class="image_wrapper_block">
								...
								<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
									...
									<?if( !empty($arItem["PREVIEW_PICTURE"]) ):?>
                                        <?$img = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"]["ID"], array( "width" => 300, "height" => 300 ), BX_RESIZE_IMAGE_PROPORTIONAL_ALT,true );?>
										<img class="lazy" data-src="<?=$img["src"]?>" src="" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
