...
$strReturn = '';
//if($_SERVER['REMOTE_ADDR']=='95.105.86.47') {\bitrix\main\diag\debug::dumpToFile($arResult);}
if($arResult){
	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			$strReturn .= '<span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a href="#" onClick="return false;" itemprop="item"><span itemprop="name">'.$title.'</a><meta itemprop="position" content="'.($index + 1).'"></span></span>';
		}
	}
