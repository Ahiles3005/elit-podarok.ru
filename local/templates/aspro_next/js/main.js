...
$(document).ready(function(){
	...
	//ecommerce order
	if(typeof arNextOptions !== undefined && arNextOptions["PAGES"]["ORDER_PAGE"])
	{
		...
		if(!th.hasClass('loading'))
		{
			...
			$.ajax({
				...
				success: function(html){
					if($('.ajax_load').length)
					{
						let block = $(html).find('.item_block');
						let list = $(html).find('.list_item_wrapp');
						let module = $(html).find('tr.main_item_wrapper');
						
						th.removeClass('loading');
						if($('.display_list').length){
							$('.display_list').append((list.length ? list : html));
							}else if($('.block_list').length){
							$('.block_list').append((block.length ? block : html));
							touchItemBlock('.catalog_item a');
							}else if($('.module_products_list').length){
							$('.module_products_list > tbody').append((module.length ? module : html));
						}
						setStatusButton();
						initCountdown();
