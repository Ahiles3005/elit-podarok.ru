...
if(!funcDefined('setLocationSKU')){function setLocationSKU(ID){var objUrl=parseUrlQuery(),j=0,prefix='',query_string='',url='';if('oid' in objUrl){if(parseInt(objUrl.oid)>0){objUrl.oid=ID;for(var i in objUrl){if(parseInt(j)>0)
    prefix='&';query_string=query_string+prefix+i+'='+objUrl[i];j++}
    if(query_string){url=location.pathname+'?'+query_string}
    try
    {history.pushState(null,null,url);return}catch(e){}
...
$(document).ready(function(e){$('img.lazy').lazy({effect:'fadeIn',delay:0})});$(document).on('click',"#basket_toolbar_button",function(){if(lastHash)
    location.hash='cart'})
...
$.fn.equalizeHeights=function(outer,classNull,minHeight){var maxHeight=this.map(function(i,e){var minus_height=0,calc_height=0;if(classNull!==!1){if(!isMobile)
...
$.fn.getFloatWidth=function(){var width=0
    ...
    return width}
$.fn.sliceHeight=function(options){function _slice(el){el.each(function(){$(this).css('line-height','');$(this).css('height','')});if(options.mobile==!0){if(window.matchMedia('(max-width: 550px)').matches)
    return}
    ...
    if(options.customSlice){var arBreakpoints=Object.keys(options.breakpoint),bSliceNext=!1;if(arBreakpoints.length){elw=(typeof(options.item)!=='undefined'&&options.item.length)?$(options.item).last().getFloatWidth():(el.last().hasClass('item')?el.last().getFloatWidth():el.last().parents('.item').getFloatWidth());if(elw)
        options.sliceNext=Math.floor(elsw/elw);for(var key in arBreakpoints){if(window.matchMedia(arBreakpoints[key].toString()).matches){bSliceNext=!0;options.slice=options.breakpoint[arBreakpoints[key]]}}}}
    ...
    var options=$.extend({slice:null,sliceNext:null,outer:!1,lineheight:!1,autoslicecount:!0,classNull:!1,minHeight:!1,row:!1,item:!1,typeResize:!1,typeValue:!1,fixWidth:0,resize:!0,mobile:!1,customSlice:!1,breakpoint:{},},options);var el=$(this);ignoreResize.push(!0);_slice(el);ignoreResize.pop();if(options.resize){BX.addCustomEvent('onWindowResize',function(eventdata){try{ignoreResize.push(!0);_slice(el)}catch(e){}finally{ignoreResize.pop()}})}else{if(!ignoreResize.length){_slice(el)}}}
...
if(!funcDefined('onLoadjqm')){var onLoadjqm=function(name,hash,requestData,selector,requestTitle,isButton,thButton){$.each($(hash.t).get(0).attributes,function(index,attr){if(/^data\-autoload\-(.+)$/.test(attr.nodeName)){var key=attr.nodeName.match(/^data\-autoload\-(.+)$/)[1];var el=$('input[data-sid="'+key.toUpperCase()+'"]');el.val(BX.util.htmlspecialcharsback($(hash.t).data('autoload-'+key))).attr('readonly','readonly');el.closest('.form-group').addClass('input-filed');el.attr('title',el.val())}});if(hash.w.hasClass('send_gift_frame')){var imgHtml=priceHtml=propsHtml='';if($('.offers_img a').length)
    ...
    var width=$('.'+name+'_frame').width();$('.'+name+'_frame').css('margin-left','-'+width/2+'px');if(name=='order-popup-call'){}else if(name=='order-button'){$(".order-button_frame").find("div[product_name]").find("input").val(hash.t.title).attr("readonly","readonly").css({"overflow":"hidden","text-overflow":"ellipsis"})}else if(name=='basket_error'){$(".basket_error_frame .pop-up-title").text(requestTitle);$(".basket_error_frame .ajax_text").html(requestData);if(window.matchMedia('(max-width: 991px)').matches){$("body").addClass("all_viewed")}
        initSelects(document);if(isButton=="Y"&&thButton)
            $("<div class='popup_button_basket_wr'><span class='popup_button_basket big_btn button' data-item="+thButton.data("item")+"><span class='btn btn-default'>"+BX.message("ERROR_BASKET_BUTTON")+"</span></span></div>").insertAfter($(".basket_error_frame .ajax_text"));}else if(name=='one_click_buy'){$('#one_click_buy_form').submit(function(){if($('#one_click_buy_form').valid()){if($('.'+name+'_frame form input.error').length||$('.'+name+'_frame form textarea.error').length){return!1}else if(!$(this).find('#one_click_buy_form_button').hasClass('clicked')){if(!$(this).find('#one_click_buy_form_button').hasClass("clicked"))
        $(this).find('#one_click_buy_form_button').addClass("clicked");var form_url=$(this).attr('action');var bSend=!0;if(window.renderRecaptchaById&&window.asproRecaptcha&&window.asproRecaptcha.key){if(window.asproRecaptcha.params.recaptchaSize=='invisible'&&typeof grecaptcha!='undefined'){if($('#one_click_buy_form').find('.g-recaptcha-response').val()){bSend=!0}else{grecaptcha.execute($('#one_click_buy_form').find('.g-recaptcha').data('widgetid'));$(this).find('#one_click_buy_form_button').removeClass("clicked");bSend=!1}}}
        if(bSend){$.ajax({url:$(this).attr('action'),data:$(this).serialize(),type:'POST',dataType:'json',error:function(data){alert('Error connecting server')},success:function(data){if(data.result=='Y'){if(arNextOptions.COUNTERS.USE_1CLICK_GOALS!=='N'){var eventdata={goal:'goal_1click_success'};BX.onCustomEvent('onCounterGoals',[eventdata])}
                if(ocb_files.length){var obData=new FormData(),bHasFiles=!1;$.each(ocb_files,function(key,value){if(value){bHasFiles=!0;obData.append(key+'_'+value.code,value[0])}});if(bHasFiles){$.ajax({url:form_url+'?uploadfiles&orderID='+data.message,type:'POST',data:obData,cache:!1,dataType:'json',processData:!1,contentType:!1,error:function(data,exception){if(data){console.log(data);console.log(exception)}
        ...
        return!1})}else if(name=='one_click_buy_basket'){$('#one_click_buy_form').on("submit",function(){if($('#one_click_buy_form').valid()){if($('.'+name+'_frame form input.error').length||$('.'+name+'_frame form textarea.error').length){return!1}else if(!$(this).find('#one_click_buy_form_button').hasClass('clicked')){if(!$(this).find('#one_click_buy_form_button').hasClass("clicked"))
        $(this).find('#one_click_buy_form_button').addClass("clicked");var form_url=$(this).attr('action');var bSend=!0;if(window.renderRecaptchaById&&window.asproRecaptcha&&window.asproRecaptcha.key){if(window.asproRecaptcha.params.recaptchaSize=='invisible'&&typeof grecaptcha!='undefined'){if($('#one_click_buy_form').find('.g-recaptcha-response').val()){bSend=!0}else{grecaptcha.execute($('#one_click_buy_form').find('.g-recaptcha').data('widgetid'));$(this).find('#one_click_buy_form_button').removeClass("clicked");bSend=!1}}}
        if(bSend){$.ajax({url:$(this).attr('action'),data:$(this).serialize(),type:'POST',dataType:'json',error:function(data){window.console&&console.log(data)},success:function(data){if(data.result=='Y'){if(arNextOptions.COUNTERS.USE_FASTORDER_GOALS!=='N'){var eventdata={goal:'goal_fastorder_success'};BX.onCustomEvent('onCounterGoals',[eventdata])}
                if(ocb_files.length){var obData=new FormData(),bHasFiles=!1;$.each(ocb_files,function(key,value){if(value){bHasFiles=!0;obData.append(key+'_'+value.code,value[0])}});if(bHasFiles){$.ajax({url:form_url+'?uploadfiles&orderID='+data.message,type:'POST',data:obData,cache:!1,dataType:'json',processData:!1,contentType:!1,error:function(data){alert('Error with files')},success:function(respond,textStatus,jqXHR){$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}})}else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.ONE_CLICK)}}else{$('.one_click_buy_result').show();$('.one_click_buy_result_success').show();purchaseCounter(data.message,arNextOptions.COUNTERS.TYPE.QUICK_ORDER)}}else{$('.one_click_buy_result').show();$('.one_click_buy_result_fail').show();if(('err' in data)&&data.err)
...
if(!funcDefined('onHidejqm')){var onHidejqm=function(name,hash){if(hash.w.find('.one_click_buy_result_success').is(':visible')&&name=="one_click_buy_basket"){window.location.href=window.location.href}
    if($('.xzoom-source').length)
        ...
        $('.xzoom-preview').remove();hash.w.animate({'opacity':0},200,function(){hash.w.hide();hash.w.empty();hash.o.remove();hash.w.removeClass('show');if(window.matchMedia('(max-width: 991px)').matches){$("body").removeClass("all_viewed")}
        if(name=='fast_view')
...
InitZoomPict=function(el){var block=$('.zoom_picture');if(typeof el!=='undefined')
    block=el;if(block.length){var slide=block.closest('.slides');var zoomer=block,options,defaults={zoomWidth:200,zoomHeight:200,adaptive:!1,title:!0,Xoffset:15,};var config=$.extend({},defaults,options,zoomer.data('plugin-options'));zoomer.xzoom(config)}}
...
CheckHeaderFixed=function(){var header_fixed=$('#headerfixed');header=$('header').first();if(header_fixed.length){if(header.length){var isHeaderFixed=!1,isTabsFixed=!1,headerCanFix=!0,headerFixedHeight=header_fixed.actual('outerHeight'),headerNormalHeight=header.actual('outerHeight'),headerDiffHeight=headerNormalHeight-headerFixedHeight,mobileBtnMenu=$('.btn.btn-responsive-nav'),headerTop=$('#panel:visible').actual('outerHeight');topBlock=$('.TOP_HEADER').first();if(headerDiffHeight<=0)
    headerDiffHeight=0;if(topBlock.length)
    headerTop+=topBlock.actual('outerHeight');$(window).scroll(function(){var tabs_fixed=$('.product-item-detail-tabs-container-fixed');if(window.matchMedia('(min-width:992px)').matches){var scrollTop=$(window).scrollTop();headerCanFix=!mobileBtnMenu.is(':visible');var tabs_offset=$('.tabs_section .nav.nav-tabs').offset();var current_is=$('.search-wrapper .search-input:visible'),title_search_result=$('.title-search-result.'+current_is.attr('id')),pos,pos_input;if(!isHeaderFixed){if((scrollTop>headerNormalHeight+headerTop)&&headerCanFix){isHeaderFixed=!0;header_fixed.addClass('fixed');if($('nav.mega-menu.sliced').length)
    $('nav.mega-menu.sliced').removeClass('initied');CheckTopMenuDotted()}}else if(isHeaderFixed||!headerCanFix){if((scrollTop<=headerDiffHeight+headerTop)||!headerCanFix){isHeaderFixed=!1;header_fixed.removeClass('fixed')}}
    ...
    var mfixed=$('.wrapper1.mfixed_Y #mobileheader');if(mfixed.length&&isMobile){var isMHeaderFixed=!1,mheaderCanFix=!0,mheaderFixedHeight=0,mheaderTop=$('#panel:visible').actual('outerHeight');$(window).scroll(function(){var scrollTop=$(window).scrollTop();if(window.matchMedia('(max-width:991px)').matches){if($('.wrapper1.mfixed_Y.mfixed_view_scroll_top #mobileheader').length){if(scrollTop>startScroll){$('.wrapper1.mfixed_Y.mfixed_view_scroll_top #mobileheader').removeClass('fixed')}else if(scrollTop>mheaderFixedHeight+mheaderTop)
        $('.wrapper1.mfixed_Y.mfixed_view_scroll_top #mobileheader').addClass('fixed');else if(scrollTop<=mheaderFixedHeight+mheaderTop)
        $('.wrapper1.mfixed_Y.mfixed_view_scroll_top #mobileheader').removeClass('fixed');startScroll=scrollTop}else{if(!isMHeaderFixed){if((scrollTop>mheaderFixedHeight+mheaderTop)){isMHeaderFixed=!0;mfixed.addClass('fixed')}}else if(isMHeaderFixed){if((scrollTop<=mheaderFixedHeight+mheaderTop)){isMHeaderFixed=!1;mfixed.removeClass('fixed')}}}}else mfixed.removeClass('fixed')})}}
...
scrollPreviewBlock=function(){if(typeof($.cookie('scroll_block'))!='undefined'&&$.cookie('scroll_block')){var scroll_block=$($.cookie('scroll_block'));if(scroll_block.length){$('body, html').animate({scrollTop:scroll_block.offset().top},500)}
...
SetFixedAskBlock=function(){if($('.ask_a_question_wrapper').length){var offset=$('.ask_a_question_wrapper').offset(),footer_offset=0,block=$('.ask_a_question_wrapper').find('.ask_a_question'),block_offset=BX.pos(block[0]),block_height=block_offset.bottom-block_offset.top,diff_top_scroll=$('#headerfixed').height()+20;if($('footer').length)
    ...
    block.css({'width':$('.fixed_block_fix').width()})}else block.removeClass('fixed').css({'top':0,'width':'auto'})}}}
...
if(!funcDefined('setPriceItem')){var setPriceItem=function setPriceItem(main_block,quantity,rewrite_price,check_quantity,is_sku,show_percent,percent){var old_quantity=main_block.find('.to-cart').attr('data-ratio'),value=(typeof rewrite_price!=='undefined'&&rewrite_price?rewrite_price:main_block.find('.to-cart').attr('data-value')),currency=main_block.find('.to-cart').attr('data-currency'),total_block='<div class="total_summ" style="display:none;"><div>'+BX.message('TOTAL_SUMM_ITEM')+'<span></span></div></div>',price_block=main_block.find('.cost.prices'),use_percent=(typeof show_percent!=='undefined'&&show_percent=='Y'),percent_number=(typeof percent!=='undefined'&&percent),check=(typeof check_quantity!=='undefined'&&check_quantity);if(main_block.find('.buy_block').length){if(!main_block.find('.buy_block .total_summ').length&&!is_sku)
    ...
    if(main_block.find('.total_summ').length){if(value&&currency){if((1==quantity&&old_quantity==quantity)||(typeof is_sku!=='undefined'&&is_sku&&!check)){main_block.find('.total_summ').slideUp(50)}else{main_block.find('.total_summ span').text(BX.Currency.currencyFormat((value*quantity),currency,!0));if(main_block.find('.total_summ').is(':hidden'))
...
if(!funcDefined('getCurrentPrice')){var getCurrentPrice=function getCurrentPrice(price,currency,print_price){var val='';var format_value=BX.Currency.currencyFormat(price,currency);if(print_price.indexOf(format_value)>=0){val=print_price.replace(format_value,'<span class="price_value">'+format_value+'</span><span class="price_currency">');val+='</span>'}else{val=print_price}
    return val}}
$(document).ready(function(){CheckSearchWidth();MegaMenuFixed();if(typeof arNextOptions!==undefined&&arNextOptions.PAGES.ORDER_PAGE){var arUrl=parseUrlQuery();if("ORDER_ID" in arUrl){var _id=arUrl.ORDER_ID;if(arNextOptions.COUNTERS.USE_FULLORDER_GOALS!=='N'){var eventdata={goal:'goal_order_success',result:_id};BX.onCustomEvent('onCounterGoals',[eventdata])}
    if(checkCounters()){if(typeof BX.localStorage!=='undefined'){var d=BX.localStorage.get('gtm_e_'+_id);if(typeof d==='object'){window.dataLayer=window.dataLayer||[];dataLayer.push({"ecommerce":d})}
    ...
    if($mobileMenu.length){$mobileMenu.isLeftSide=$mobileMenu.hasClass('leftside')
        ...
        if($mobileMenu.isLeftSide){$mobileMenu.parent().append('<div id="mobilemenu-overlay"></div>')
            ...
            $mobileMenuOverlay.click(function(){if($mobileMenu.isOpen){CloseMobileMenu()}});$(document).swiperight(function(e){if(!$(e.target).closest('.flexslider').length&&!$(e.target).closest('.swipeignore').length){OpenMobileMenu()}});$(document).swipeleft(function(e){if(!$(e.target).closest('.flexslider').length&&!$(e.target).closest('.swipeignore').length){CloseMobileMenu()}})}else{$('#mobileheader').click(function(e){if(!$(e.target).closest('#mobilemenu').length&&!$(e.target).closest('.burger').length&&$mobileMenu.isOpen){CloseMobileMenu()}})}
        $('#mobilemenu .menu a,#mobilemenu .social-icons a').click(function(e){var $this=$(this)
            if($this.hasClass('parent')){e.preventDefault()
                if(!$mobileMenu.isDowndrop){$this.closest('li').addClass('expanded')
                    MoveMobileMenuWrapNext()}else{if(!$this.closest('li').hasClass('expanded')){$this.closest('li').addClass('expanded')}else{$this.closest('li').removeClass('expanded')}}}else{if($this.closest('li').hasClass('counters')){var href=$this.attr('href')
        ...
        OpenMobileMenu=function(){if(!$mobileMenu.isOpen){if($('.style-switcher').hasClass('active')){$('.style-switcher .switch').trigger('click')}
            ...
            if($mobileMenu.isLeftSide){setTimeout(function(){$mobileMenuOverlay.fadeIn('fast')},100)}else{$('body').scrollTop(0).css({position:'fixed'})
        ...
        CloseMobileMenu=function(){if($mobileMenu.isOpen){$mobileMenu.removeClass('show')
            ...
            if($mobileMenu.isLeftSide){setTimeout(function(){$mobileMenuOverlay.fadeOut('fast')},100)}else{$('#mobileheader .burger').removeClass('c')
                $('body').css({position:''})}
    ...
    $(document).on('keyup','.search-input-div input',function(e){var inputValue=$(this).val();$('.search-input-div input:not(:focus').val(inputValue);if($(this).closest('#headerfixed').length){if(e.keyCode==13)
        $('.search form').submit();}});$(document).on('click','.search-button-div button',function(e){if($(this).closest('#headerfixed').length)
        $('.search form').submit();});$('.inline-search-show, .inline-search-hide').on('click',function(e){if(typeof($(this).data('type_search'))!='undefined'&&$(this).data('type_search')=='fixed')
        $('.inline-search-block').addClass('fixed');if(arNextOptions.THEME.TYPE_SEARCH!='fixed'){var height_block=0;if($('header.long').length){height_block=$('header.long').closest('#header').actual('outerHeight')}else{height_block=$(this).closest('.maxwidth-theme').actual('outerHeight');if($(this).closest('.top-block').length)
        ...
        if(arNextOptions.THEME.TYPE_SEARCH=='fixed'){if($('.inline-search-block').hasClass('show'))
    ...
    $("html, body").on('mousedown',function(e){if(typeof e.target.className=='string'&&e.target.className.indexOf('adm')<0){e.stopPropagation();var search_target=$(e.target).closest('.bx_searche');if(!$(e.target).hasClass('inline-search-block')&&!$(e.target).hasClass('svg')&&!search_target.length){$('.inline-search-block').removeClass('show');$('.title-search-result').hide();if(arNextOptions.THEME.TYPE_SEARCH=='fixed')
        ...
        if(isMobile){if(search_target.length)
            location.href=search_target.attr('href');}
        var class_name=$(e.target).attr('class');if(typeof(class_name)=='undefined'||class_name.indexOf('tooltip')<0)
            $('.tooltip-link').tooltip('hide');}});$('.inline-search-block').find('*').on('mousedown',function(e){e.stopPropagation()});if(jQuery.browser.mobile){$(document).on('click','*[data-event="jqm"]',function(e){e.preventDefault();e.stopPropagation();var _this=$(this);var name=_this.data('name');if(window.matchMedia('(min-width:992px)').matches){if(!$(this).hasClass('clicked')){$(this).addClass('clicked');$(this).jqmEx();$(this).trigger('click')}
        return!1}else if(name.length){var script=arNextOptions.SITE_DIR+'form/';var paramsStr='';var arTriggerAttrs={};$.each(_this.get(0).attributes,function(index,attr){var attrName=attr.nodeName;var attrValue=_this.attr(attrName);arTriggerAttrs[attrName]=attrValue;if(/^data\-param\-(.+)$/.test(attrName)){var key=attrName.match(/^data\-param\-(.+)$/)[1];paramsStr+=key+'='+attrValue+'&'}});var triggerAttrs=JSON.stringify(arTriggerAttrs);var encTriggerAttrs=encodeURIComponent(triggerAttrs);script+='?name='+name+'&'+paramsStr+'data-trigger='+encTriggerAttrs;location.href=script}});$('.fancybox').removeClass('fancybox')}else{$(document).on('click','*[data-event="jqm"]',function(e){e.preventDefault();e.stopPropagation();if(!$(this).hasClass('clicked')){$(this).addClass('clicked');$(this).jqmEx();$(this).trigger('click')}
        return!1})}
    BX.addCustomEvent('onCompleteAction',function(eventdata,_this){try{if(eventdata.action==='loadForm'){$(_this).parent().removeClass('loadings');$(_this).removeClass('clicked');if($(_this).hasClass('one_click_buy_trigger'))
        ...
        $('.basket-link.delay .count').text(arBasketPrices.DELAY_COUNT)}else if(eventdata.action==='loadActualBasketCompare'){var compare_count=Object.keys(arBasketAspro.COMPARE).length;if(compare_count>0){$('.basket-link.compare').addClass('basket-count');$('.basket-link.compare .count').removeClass('empted');if($('#compare_fly').length)
        ...
        $('.basket-link.compare .count').text(compare_count)}else if(eventdata.action==='loadRSS'){}}catch(e){console.error(e)}})
    ...
    InitFlexSlider();setTimeout(function(){InitTopestMenuGummi();isOnceInited=!0},50);InitZoomPict();$(document).on('click','.captcha_reload',function(e){var captcha=$(this).parents('.captcha-row');e.preventDefault();$.ajax({url:arNextOptions.SITE_DIR+'ajax/captcha.php'}).done(function(text){captcha.find('input[name=captcha_sid]').val(text);captcha.find('img').attr('src','/bitrix/tools/captcha.php?captcha_sid='+text);captcha.find('input[name=captcha_word]').val('').removeClass('error');captcha.find('.captcha_input').removeClass('error').find('.error').remove()})});if(arNextOptions.PAGES.BASKET_PAGE){if(arNextOptions.THEME.SHOW_BASKET_PRINT=="Y"){if($('.page-top h1').length)
    ...
    $(document).on("change",".counter_block input[type=text]",function(e){if(!$(this).parents('.basket_wrapp').length){var val=$(this).val(),tmp_ratio=$(this).parents(".counter_wrapp").find(".to-cart").data('ratio')?$(this).parents(".counter_wrapp").find(".to-cart").data('ratio'):$(this).parents('tr').first().find("td.buy .to-cart").data('ratio'),isDblQuantity=$(this).parents(".counter_wrapp").find(".to-cart").data('float_ratio')?$(this).parents(".counter_wrapp").find(".to-cart").data('float_ratio'):$(this).parents('tr').first().find("td.buy .to-cart").data('float_ratio'),ratio=(isDblQuantity?parseFloat(tmp_ratio):parseInt(tmp_ratio,10)),diff=val%ratio;if(isDblQuantity){ratio=Math.round(ratio*arNextOptions.JS_ITEM_CLICK.precisionFactor)/arNextOptions.JS_ITEM_CLICK.precisionFactor;if(Math.round(diff*arNextOptions.JS_ITEM_CLICK.precisionFactor)/arNextOptions.JS_ITEM_CLICK.precisionFactor==ratio)
        ...
        try{if(typeof eventdata.type!='undefined'){if(!eventdata.params.id.closest('.gifts').length){var obProduct=eventdata.params.id.data('product');if(typeof window[obProduct]=='object'){window[obProduct].setPriceAction('Y')}else if(eventdata.params.id.length){if(eventdata.params.id.closest('.main_item_wrapper').length&&arNextOptions.THEME.SHOW_TOTAL_SUMM=='Y'){setPriceItem(eventdata.params.id.closest('.main_item_wrapper'),eventdata.params.value)}}
    ...
    $(document).on('click','.to-cart:not(.read_more)',function(e){e.preventDefault();var th=$(this);if(!th.hasClass('clicked')){th.addClass('clicked');var val=$(this).attr('data-quantity');var tmp_props=$(this).data("props"),props='',part_props='',add_props='N',fill_prop={},iblockid=$(this).data('iblockid'),offer=$(this).data('offers'),rid='',basket_props='',item=$(this).attr('data-item');if(th.closest('.but-cell').length){if($('.counter_block[data-item="'+item+'"]').length)
    ...
    $(document).on('click','.wish_item',function(e){e.preventDefault();var val=$(this).attr('data-quantity'),_this=$(this),offer=$(this).data('offers'),iblockid=$(this).data('iblock'),tmp_props=$(this).data("props"),props='',item=$(this).attr('data-item');item2=$(this).attr('data-item');if(!_this.hasClass('clicked')){_this.addClass('clicked');if(!val)$val=1;if(offer!="Y")offer="N";if(tmp_props){props=tmp_props.split(";")}
    ...
    $(document).on('mouseenter','.display_list .item_wrap',function(){$(this).prev().addClass('prev')});$(document).on('mouseleave','.display_list .item_wrap',function(){$(this).prev().removeClass('prev')});$(document).on('click','.no_goods .button',function(){$('.bx_filter .smartfilter .bx_filter_search_reset').trigger('click')});$(document).on('click','.ajax_load_btn',function(){var url=$(this).closest('.container').find('.module-pagination .flex-direction-nav .flex-next').attr('href'),th=$(this).find('.more_text_ajax');if(!th.hasClass('loading')){th.addClass('loading');var objUrl=parseUrlQuery(),add_url=''
        obGetData={"ajax_get":"Y","AJAX_REQUEST":"Y"};if('clear_cache' in objUrl){if(objUrl.clear_cache=='Y')
            add_url='&clear_cache=Y'}
        if('q' in objUrl){add_url="?q="+objUrl.q+"&s="+objUrl.s}
        if(th.closest('.goods-block.ajax_load'))
            obGetData.bitrix_include_areas='N';$.ajax({url:url+add_url,data:obGetData,success:function(html){if($('.ajax_load').length){let block=$(html).find('.item_block');let list=$(html).find('.list_item_wrapp');let module=$(html).find('tr.main_item_wrapper');th.removeClass('loading');if($('.display_list').length){$('.display_list').append((list.length?list:html))}else if($('.block_list').length){$('.block_list').append((block.length?block:html));touchItemBlock('.catalog_item a')}else if($('.module_products_list').length){$('.module_products_list > tbody').append((module.length?module:html))}
                setStatusButton();initCountdown();var eventdata={action:'ajaxContentLoadedTab'};BX.onCustomEvent('onAjaxSuccess',[eventdata]);$('.bottom_nav').html($(html).find('.bottom_nav').html());$('img.lazy').lazy({effect:'fadeIn',delay:0})}else{if($('.banners-small.front').length){$('.banners-small .items.row').append(html);$('.bottom_nav').html($('.banners-small .items.row .bottom_nav').html());$('.banners-small .items.row .bottom_nav').remove()}else{if(th.closest('.item-views').find('.items').length){th.closest('.item-views').find('.items').append(html)}else{$(html).insertBefore($('.blog .bottom_nav'))}
    ...
    showPhoneMask('input[autocomplete=tel]');BX.addCustomEvent(window,"onAjaxSuccess",function(e){if(e!='OK'){initSelects(document);InitOrderCustom();showPhoneMask('input[autocomplete=tel]');if($('.catalog_detail').length&&!$('.fast_view_frame').length){$('.bx_filter').remove();InitFlexSlider()}
        if(arNextOptions.PAGES.CATALOG_PAGE){initCountdown()}
    ...
    BX.addCustomEvent(window,"onFrameDataRequestFail",function(response){console.log(response)})});if(!funcDefined('setBasketStatusBtn')){setBasketStatusBtn=function(){if(typeof(arBasketAspro)!=='undefined'){if('BASKET' in arBasketAspro){if(arBasketAspro.BASKET){for(var i in arBasketAspro.BASKET){$('.to-cart[data-item='+i+']').hide();$('.counter_block[data-item='+i+']').hide();$('.in-cart[data-item='+i+']').show();$('.in-cart[data-item='+i+']').closest('.button_block').addClass('wide')}}}
...
function showTotalSummItem(popup){if(arNextOptions.THEME.SHOW_TOTAL_SUMM_TYPE=="ALWAYS"&&arNextOptions.THEME.SHOW_TOTAL_SUMM=="Y"){var parent='body ';if(typeof popup==='string'&&popup=='Y')
    parent='.popup ';$(parent+'.counter_wrapp .counter_block input.text').each(function(){var _th=$(this);if(_th.data('product')){var obProduct=_th.data('product');if(typeof window[obProduct]=='object')
    window[obProduct].setPriceAction('Y');else setPriceItem(_th.closest('.main_item_wrapper'),_th.val())}else setPriceItem(_th.closest('.main_item_wrapper'),_th.val())})}}
...
if(!funcDefined('orderActions')){orderActions=function(e){if(arNextOptions.PAGES.ORDER_PAGE){if($('#bx-soa-order input[autocomplete=tel]').length){for(var i=0;i<BX.Sale.OrderAjaxComponent.result.ORDER_PROP.properties.length;++i){if(BX.Sale.OrderAjaxComponent.result.ORDER_PROP.properties[i].IS_PHONE=='Y'){var arPropertyPhone=BX.Sale.OrderAjaxComponent.result.ORDER_PROP.properties[i]}}
    ...
    if($('.bx-soa-cart-total').length){if(!$('.change_basket').length)
        ...
        if($('.bx-soa-cart-total-line-total').length&&!$('.licence_block.filter').length&&arNextOptions.THEME.SHOW_LICENCE=="Y"){if(typeof(e)==='undefined')
            ...
            $('.bx-soa .licence_block label.license').on('click',function(){var id=$(this).data('for');$('.bx-soa .licence_block label.error').addClass('hidden');if(!$('input[name='+id+']').prop('checked')){$('input[name='+id+']').prop('checked','checked');BX.Sale.OrderAjaxComponent.state_licence='checked'}else{$('input[name='+id+']').prop('checked','');BX.Sale.OrderAjaxComponent.state_licence=''}})
            $('.lic_condition a').on('click',function(){var iCountErrors=BX.Sale.OrderAjaxComponent.isValidPropertiesBlock().length;if(!BX.Sale.OrderAjaxComponent.activeSectionId||!iCountErrors){BX.Sale.OrderAjaxComponent.animateScrollTo($('.licence_block')[0],800,50)}})}
        BX.removeClass(BX.Sale.OrderAjaxComponent.totalInfoBlockNode,'bx-soa-cart-total-fixed');$(window).scroll();if(checkCounters()&&typeof(BX.Sale.OrderAjaxComponent.oldSaveOrder)==="undefined"&&typeof(BX.Sale.OrderAjaxComponent.saveOrder)!=='undefined'){BX.Sale.OrderAjaxComponent.oldSaveOrder=BX.Sale.OrderAjaxComponent.saveOrder;BX.Sale.OrderAjaxComponent.saveOrder=function(result){var res=BX.parseJSON(result);if(res&&res.order){if(!res.order.SHOW_AUTH){if(res.order.REDIRECT_URL&&res.order.REDIRECT_URL.length&&(!res.order.ERROR||BX.util.object_keys(res.order.ERROR).length<1)){if((arMatch=res.order.REDIRECT_URL.match(/ORDER_ID\=[^&=]*/g))&&arMatch.length&&(_id=arMatch[0].replace(/ORDER_ID\=/g,'',arMatch[0]))){$.ajax({url:arNextOptions.SITE_DIR+"ajax/check_order.php",dataType:"json",type:"POST",data:{"ID":_id},success:function(id){if(parseInt(id)){purchaseCounter(parseInt(id),BX.message('FULL_ORDER'),function(d){if(typeof BX.localStorage!=='undefined'){BX.localStorage.set('gtm_e_'+_id,d,60)}
                BX.Sale.OrderAjaxComponent.oldSaveOrder(result)})}},error:function(){BX.Sale.OrderAjaxComponent.oldSaveOrder(result)}})}else{BX.Sale.OrderAjaxComponent.oldSaveOrder(result)}}else{BX.Sale.OrderAjaxComponent.oldSaveOrder(result)}}else{BX.Sale.OrderAjaxComponent.oldSaveOrder(result)}}else{BX.Sale.OrderAjaxComponent.oldSaveOrder(result)}}}}
...
if(!funcDefined('basketActions')){basketActions=function(){if(arNextOptions.PAGES.BASKET_PAGE){checkMinPrice();if(typeof(BX.Sale)!=='undefined'&&typeof(BX.Sale)==='object'){if(typeof(BX.Sale.BasketComponent)!=='undefined'&&typeof(BX.Sale.BasketComponent)==='object'){$(document).on('click','.basket-item-actions-remove',function(){var basketID=$(this).closest('.basket-items-list-item-container').data('id');delFromBasketCounter(BX.Sale.BasketComponent.items[basketID].PRODUCT_ID)})}}
    if(location.hash){var hash=location.hash.substring(1);if($('#basket_toolbar_button_'+hash).length){$('#basket_toolbar_button_'+hash).trigger('click')}}
...
var timerResize=!1,ignoreResize=[];$(window).resize(function(){CheckPopupTop();if(!ignoreResize.length){if(timerResize){clearTimeout(timerResize);timerResize=!1}
    ...
    $(".bx_filter_section .bx_filter_select_container").each(function(){var prop_id=$(this).closest('.bx_filter_parameters_box').attr('property_id');if($('#smartFilterDropDown'+prop_id).length){$('#smartFilterDropDown'+prop_id).css("max-width",$(this).width())}})}catch(e){}finally{ignoreResize.pop()}});BX.addCustomEvent('onWindowScroll',function(eventdata){try{ignoreScroll.push(!0)}catch(e){}finally{ignoreScroll.pop()}});BX.addCustomEvent('onSlideInit',function(eventdata){try{ignoreResize.push(!0);if(eventdata){var slider=eventdata.slider;if(slider){if(slider.hasClass('small-gallery'))
    ...
    curSlide.addClass('shown')}}}catch(e){}finally{ignoreResize.pop()}});BX.addCustomEvent('onCounterGoals',function(eventdata){if(arNextOptions.THEME.YA_GOALS=='Y'&&arNextOptions.THEME.YA_COUNTER_ID){var idCounter=arNextOptions.THEME.YA_COUNTER_ID;idCounter=parseInt(idCounter);if(typeof eventdata!='object')
    eventdata={goal:'undefined'};if(typeof eventdata.goal!='string')
    eventdata.goal='undefined';if(idCounter){try
{waitCounter(idCounter,50,function(){var obCounter=window['yaCounter'+idCounter];if(typeof obCounter=='object'){obCounter.reachGoal(eventdata.goal)}})}catch(e){console.error(e)}}else{console.info('Bad counter id!',idCounter)}}})
...
BX.addCustomEvent('onSubmitForm',function(eventdata){try{if(!window.renderRecaptchaById||!window.asproRecaptcha||!window.asproRecaptcha.key){eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending');return!0}
    if(window.asproRecaptcha.params.recaptchaSize=='invisible'&&typeof grecaptcha!='undefined'){if($(eventdata.form).find('.g-recaptcha-response').val()){eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending')}else{grecaptcha.execute($(eventdata.form).find('.g-recaptcha').data('widgetid'));return!1}}else{eventdata.form.submit();$(eventdata.form).closest('.form').addClass('sending')}
